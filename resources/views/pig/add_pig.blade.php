...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const bindPresetActions = function () {
            const presetButtons = presetContainer.querySelectorAll('[data-fill-stage]');
            presetButtons.forEach(function (button) {
                button.addEventListener('click', function () {
                    const growthName = button.getAttribute('data-fill-stage') ?? '';
        ...
        const renderPresets = function (growthStages) {
            ...
            presetContainer.innerHTML = growthStages
                .map(function (growthStage) {
                    ...
                    return `<button type="button" data-fill-stage="${growthName}" class="rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-50">${growthName}</button>`;
                })
                .join('');
