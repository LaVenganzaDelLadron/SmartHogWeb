export type SuccessAlertOptions = {
    title: string;
    message: string;
    durationMs?: number;
...
const ALERT_ROOT_ID = 'smart-hog-success-alert-root';
...
function createAlert(options: SuccessAlertOptions): HTMLDivElement {
    ...
    progress.style.background = 'linear-gradient(90deg, #059669, #10b981)';
    ...
    iconWrap.innerHTML =
        '<svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="m4 10 4 4 8-8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
export function showSuccessAlert(options: SuccessAlertOptions): void {
...
declare global {
    interface Window {
        showSuccessAlert: (options: SuccessAlertOptions) => void;
    }
}
